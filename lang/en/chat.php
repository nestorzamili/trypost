<?php

declare(strict_types=1);

return [
    'title' => 'AI Assistant',
    'headline' => 'How can I help with your workspace?',
    'description' => 'Ask about posts, brands, labels, signatures or assets — like talking to TryPost MCP, without Claude or ChatGPT.',
    'placeholder' => 'Ask about posts, brands, labels, signatures or assets',
    'send' => 'Send',
    'stop' => 'Stop',
    'retry' => 'Try again',
    'suggestions_label' => 'Try asking about',

    'attachment' => [
        'attach' => 'Attach a CSV',
        'remove' => 'Remove attachment',
        'default_prompt' => 'Import these content briefs into drafts.',
        'errors' => [
            'file_too_large' => 'This file is too large. Attach a CSV under 1 MB.',
            'read_failed' => 'Could not read this file. Try again.',
        ],
    ],

    'brief_import' => [
        'created' => ':count drafts created from your briefs.',
        'failed' => ':count briefs could not be turned into drafts.',
        'view_drafts' => 'View drafts labelled :label',
    ],
    'suggestions' => [
        'posts' => 'Recent posts',
        'metrics' => 'Post metrics',
        'brands' => 'My brands',
        'labels' => 'Available labels',
        'signatures' => 'Active signatures',
        'assets' => 'Find assets for campaign',
    ],
    'thinking' => 'Thinking…',
    'history' => [
        'rename' => 'Rename',
        'delete' => 'Delete',
        'options' => 'Conversation options',
        'delete_title' => 'Delete conversation?',
        'delete_description' => 'This will permanently delete this conversation and its messages.',
    ],
    'groups' => [
        'today' => 'Today',
        'yesterday' => 'Yesterday',
        'last_7_days' => 'Last 7 days',
        'last_30_days' => 'Last 30 days',
        'older' => 'Older',
    ],
    'tools' => [
        'post_not_found' => 'Post not found.',
        'asset_not_found' => 'Asset not found.',
        'label_not_found' => 'Label not found.',
        'signature_not_found' => 'Signature not found.',
        'brand_variant_not_found' => 'Brand variant not found.',
        'brand_reference_not_found' => 'Brand reference photo not found.',
        'error' => 'Something went wrong. Please try again.',
        'publish_no_enabled_platforms' => 'This post has no enabled platforms. Enable at least one before publishing.',
        'delete_blocked' => "This post has already been published and can't be deleted from TryPost. If it's live on a platform that doesn't support automatic removal, like TikTok, remove it there manually.",
        'forbidden' => "You don't have permission to create or change posts in this workspace. Ask a workspace admin if you need it.",
        'admin_forbidden' => "You don't have permission to change workspace settings. Ask a workspace admin if you need it.",
    ],
    'approvals' => [
        'publish' => 'Publish this post now? It will go live immediately on every enabled platform, and some platforms — like TikTok — offer no way to unpublish afterward.',
        'delete_scheduled' => 'Deleting this post can\'t be undone. If it\'s scheduled to publish, this also cancels that. Delete anyway?',
        'delete_label' => 'Delete this label? Posts using it will lose that tag. Delete anyway?',
        'delete_signature' => 'Delete this signature? It can no longer be appended to posts. Delete anyway?',
        'delete_asset' => 'Delete this asset? The file is removed for good and posts using it keep only their own copy. Delete anyway?',
        'update_brand' => 'Change the workspace brand? Every post generated afterwards follows the new brand. Continue?',
        'delete_brand_variant' => 'Delete this brand variant? Posts generated afterwards in its language fall back to the default brand. Delete anyway?',
        'delete_brand_reference' => 'Delete this brand reference photo? It will no longer guide generated visuals. Delete anyway?',
    ],
    'errors' => [
        'turn_in_progress' => 'This conversation is still replying. Wait for it to finish before sending another message.',
        'request_failed' => 'Something went wrong sending your message. Please try again.',
        'stream_failed' => 'Something went wrong while replying. Try again.',
        'payment_required_cta' => 'Manage billing',
        'access_error' => "This conversation isn't available. It may have been deleted, or it belongs to someone else.",
    ],
    'tool_names' => [
        'list_posts' => 'List posts',
        'get_post' => 'Post details',
        'get_post_metrics' => 'Post metrics',
        'create_post' => 'Create post',
        'create_posts_from_briefs' => 'Import briefs',
        'update_post' => 'Update post',
        'schedule_post' => 'Schedule post',
        'publish_post' => 'Publish post',
        'delete_post' => 'Delete post',
        'start_post_generation' => 'Post options',
        'generate_post' => 'Generate post',
        'retry_post_images' => 'Retry images',
        'get_brand' => 'Brand details',
        'list_labels' => 'List labels',
        'list_signatures' => 'List signatures',
        'list_assets' => 'List assets',
        'get_asset' => 'Asset details',
        'attach_existing_asset' => 'Attach asset',
        'create_label' => 'Create label',
        'update_label' => 'Update label',
        'delete_label' => 'Delete label',
        'create_signature' => 'Create signature',
        'update_signature' => 'Update signature',
        'delete_signature' => 'Delete signature',
        'update_brand' => 'Update brand',
        'create_brand_variant' => 'Create brand variant',
        'update_brand_variant' => 'Update brand variant',
        'delete_brand_variant' => 'Delete brand variant',
        'delete_brand_reference_photo' => 'Delete reference photo',
        'add_brand_reference_from_url' => 'Add reference photo',
        'delete_asset' => 'Delete asset',
        'add_asset_from_url' => 'Add asset from link',
    ],
    'tool_actions' => [
        'list_posts' => 'Fetching posts…',
        'get_post' => 'Loading post details…',
        'get_post_metrics' => 'Loading metrics…',
        'create_post' => 'Creating post…',
        'create_posts_from_briefs' => 'Importing briefs…',
        'update_post' => 'Updating post…',
        'delete_post' => 'Deleting post…',
        'generate_post' => 'Generating post…',
        'schedule_post' => 'Scheduling post…',
        'publish_post' => 'Publishing post…',
        'retry_post_images' => 'Retrying images…',
        'list_labels' => 'Fetching labels…',
        'create_label' => 'Creating label…',
        'update_label' => 'Updating label…',
        'delete_label' => 'Deleting label…',
        'list_signatures' => 'Fetching signatures…',
        'create_signature' => 'Creating signature…',
        'update_signature' => 'Updating signature…',
        'delete_signature' => 'Deleting signature…',
        'get_brand' => 'Loading brand…',
        'update_brand' => 'Updating brand…',
        'create_brand_variant' => 'Creating brand variant…',
        'update_brand_variant' => 'Updating brand variant…',
        'delete_brand_variant' => 'Deleting brand variant…',
        'delete_brand_reference_photo' => 'Deleting reference photo…',
        'add_brand_reference_from_url' => 'Adding reference photo…',
        'list_assets' => 'Fetching assets…',
        'get_asset' => 'Loading asset…',
        'attach_existing_asset' => 'Attaching asset…',
        'add_asset_from_url' => 'Adding asset…',
        'delete_asset' => 'Deleting asset…',
        'start_post_generation' => 'Starting generation…',
    ],
    'tool_card' => [
        'running' => 'Running…',
        'denied' => 'You declined this action.',
        'unknown_tool' => "This assistant used a tool this interface doesn't have a card for yet.",
        'unreadable_result' => "This result couldn't be read.",
        'empty_list' => 'No posts found.',
        'empty_labels' => 'No labels found.',
        'empty_signatures' => 'No signatures found.',
        'empty_assets' => 'No assets found.',
        'open_in_editor' => 'Open in editor',
        'untitled' => 'No content yet.',
        'post_deleted' => 'This post was deleted.',
        'label_deleted' => 'This label was deleted.',
        'signature_deleted' => 'This signature was deleted.',
        'asset_deleted' => 'This asset was deleted.',
        'variant_deleted' => 'This brand variant was deleted.',
        'reference_deleted' => 'This reference photo was deleted.',
    ],
    'metrics' => [
        'unsupported' => [
            'not_published' => 'Not published yet',
            'platform_not_supported' => "Metrics aren't available for this platform",
        ],
        'empty' => 'No metrics to show yet.',
    ],
    'approval' => [
        'approve' => 'Approve',
        'reject' => 'Reject',
        'generic_reason' => 'This action needs your approval before it happens.',
    ],
    'post_generation' => [
        'unavailable' => "This workspace has no connected account yet, so there's no format to generate a post for. Connect an account first.",
        'unavailable_unsupported' => 'The connected :platforms account(s) cannot be AI-generated yet. Connect an account on a supported platform first.',
        'styles_unavailable' => "No post styles are available right now, so there's nothing to generate this format with.",
        'format_question' => 'Which format should I generate?',
        'style_question' => 'Which style should it use?',
        'images_question' => 'How many images should it have?',
        'images_none' => 'None',
        'account_question' => 'Which account is this for?',
        'language_question' => 'Which language should the post be in?',
        'topic_line' => 'About: :topic',
        'posting_to' => 'Posting to :account',
        'brand_colors_label' => 'Brand colors',
        'brand_colors_description' => 'Apply your workspace brand palette to the generated images.',
        'brand_references_label' => 'Brand references',
        'brand_references_description' => 'Use your brand reference photos to guide the generated visuals.',
        'change' => 'Change',
        'submit' => 'Generate post',
        'sent' => 'Choices sent.',
        'sentence' => 'Generate :format about :topic, :style style, :images, on :account.',
        'sentence_with_brand' => 'Generate :format about :topic, :style style, :images, on :account, :brand.',
        'sentence_language' => 'in :language',
        'sentence_references_on' => 'with brand references',
        'sentence_references_off' => 'without brand references',
        'sentence_images_none' => 'no images',
        'sentence_images_one' => '1 image',
        'sentence_images_other' => ':count images',
        'sentence_brand_on' => 'with brand colors',
        'sentence_brand_off' => 'without brand colors',
        'result_waiting' => 'Generating your post…',
        'result_text_ready' => 'Draft text is ready — rendering images…',
        'result_images_running' => 'Draft text is ready — rendering :total images…',
        'result_images_progress' => 'Draft text is ready — rendering images (:done/:total)…',
        'result_open_draft' => 'Open draft',
        'preview_show' => 'Preview',
        'preview_hide' => 'Hide preview',
        'preview_error' => "Couldn't load the preview. Open in editor instead.",
        'view_image' => 'View image',
        'result_elapsed_label' => 'Elapsed: :elapsed',
        'result_waiting_hint' => "This keeps running if you leave. The post shows up here when it's ready.",
        'result_detached_hint' => 'This keeps running in the background. Reopen this conversation to see the post.',
        'result_ready' => 'Your post is ready.',
        'result_failed' => "The post couldn't be generated. Ask again to retry.",
    ],
];
