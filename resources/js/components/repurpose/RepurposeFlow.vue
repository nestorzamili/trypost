    ...
    >
                ...
                <div class="space-y-0.5 text-xs">
                    <p class="font-semibold">
                        {{ source.label
                            ...
                            v-if="source.username"
                            class="font-normal opacity-80"
                            >&nbsp;·&nbsp;@{{ source.username }}</span
                        >
                    </p>
                    <p class="opacity-70">
                        {{
        ...
        <div v-if="destinations.length > 0" class="flex items-center gap-2">
            <Tooltip
                ...
                :key="`${destination.platform}-${index}`"
            >
                <TooltipTrigger as-child>
                    <span>
                        <PlatformLogo
                    ...
                    <div class="space-y-0.5 text-xs">
                        <p class="font-semibold">
                            {{ destination.label
                                ...
                                v-if="destination.username"
                                class="font-normal opacity-80"
                                >&nbsp;·&nbsp;@{{ destination.username }}</span
                            >
                        </p>
                        <p class="opacity-70">
                            {{ getPlatformLabel(destination.platform)
                            }}<span v-if="destination.format"
                                >&nbsp;·&nbsp;{{ destination.format }}</span
                            >
                        </p>
                    </div>
                </TooltipContent>
