...
const variables = defineModel<AutomationVariable[]>('variables', {
    default: () => [],
});
</script>

<template>
    <aside class="flex w-[26rem] flex-shrink-0 flex-col py-3 pr-3 pl-2">
        <div
        ...
        >
            ...
            <template v-if="editing">
                <div
                    class="flex items-center justify-between gap-2 border-b-2 border-foreground/10 px-3 py-3"
                >
                    <div class="flex min-w-0 items-center gap-1.5">
                        ...
                        <span class="truncate text-sm font-bold capitalize">{{
                            nodeTitle
                        }}</span>
                    </div>
                    <Button
            ...
            >
                <TabsList
                    class="flex h-auto w-full gap-2 border-b-2 border-foreground/10 px-4 py-3"
                >
                    <TabsTrigger value="build" class="flex-1">{{
                        $t('automations.tabs.build')
                    }}</TabsTrigger>
                    <TabsTrigger value="variables" class="flex-1">
                        {{ $t('automations.tabs.variables')
                        }}<span v-if="variables.length">
                            ({{ variables.length }})</span
                        >
                    </TabsTrigger>
                    <TabsTrigger value="test" class="flex-1">{{
                        $t('automations.tabs.test')
                    }}</TabsTrigger>
                </TabsList>
